...
f <- train %>% dplyr::mutate(rowIndex = 1:n())
ids <- f %>% select(id, rowIndex,type)
p <- model_delfi$pred
p <- inner_join(ids, p, by="rowIndex")
p <- p %>% select(-rowIndex)
p<-p %>% select(id,type,cancer,Resample)
#write.csv(p,here("data","stability_superscreen.csv"))
...
write.csv(res,"../data/scores/Screening_CV.csv")

saveRDS(model_delfi,"../data/models/Screening.rds")
...
write.csv(res,"../data/scores/Screening_Val.csv")
