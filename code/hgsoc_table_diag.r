...
meta2_CV<-meta2_CV %>% filter(training_set_include_diag=="yes")
meta2_Val<-meta2_Val %>% filter(diag_val=="yes")
ref_dat<-meta2_CV

meta2_CV<-meta2_CV %>% filter(classifier_type=="healthy"|subtype_simple=="HGSOC")
meta2_Val<-meta2_Val %>% filter(classifier_type=="healthy"|subtype_simple=="HGSOC")

library(pROC)

sens<-function(d,dv,spec,full_dat) {
  d3<-roc(d$classifier_type,d$`Diagnostic ZEUS`,levels=c("healthy",'cancer')) %>% coords(transpose=FALSE) %>% filter(specificity>=spec) %>% slice(1)
  d3_t<-roc(full_dat$classifier_type,full_dat$`Diagnostic ZEUS`,levels=c("healthy",'cancer')) %>% coords(transpose=FALSE) %>% filter(specificity>=spec) %>% slice(1)
  d3$threshold<-d3_t$threshold
  d3$model<-"Diagnostic DELFI+CA125+HE4"
  d3$val_sens<-(dv %>% filter(`Diagnostic ZEUS`>=d3$threshold & classifier_type=="cancer") %>% nrow())/(dv %>% filter(classifier_type=="cancer") %>% nrow())
  d3$val_spec<-(dv %>% filter(`Diagnostic ZEUS`<d3$threshold & classifier_type=="healthy") %>% nrow())/(dv %>% filter(classifier_type=="healthy") %>% nrow())
...
res$set<-"Stage IV HGSOC"
res2<-rbind(res2,res)

########

write.csv(res2,"../data/tables/AllHGSOC_diag.csv")
