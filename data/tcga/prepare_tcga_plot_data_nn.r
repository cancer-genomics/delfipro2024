...
date()
liver.thresh <- 0.30 #Same as for OV, according to Davoli et al
...
liver.losses <- liver.losses[ startsWith(aliquot, '01') | startsWith(aliquot, '02') ,]
cat('number of samples:')
print(nrow(liver.losses))
...
liver.gains <- liver.gains[startsWith(aliquot, '01') | startsWith(aliquot, '02'),]
...
fig.data <- b
saveRDS(fig.data, file = 'data.rds')
